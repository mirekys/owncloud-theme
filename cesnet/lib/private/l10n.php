...
class OC_L10N implements \OCP\IL10N {
	...
	public static function setLanguageFromRequest() {
		if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
			...
			// Ignore cs as it doesn't translate well,
			// always use cs_CZ translation rather
			$csIdx = array_search('cs', $available);
			if ($csIdx !== FALSE) { unset($available[$csIdx]); }
			...
			foreach ($preferences as $preference) {
				...
				foreach ($available as $available_language) {
					if ((substr($preferred_language, 0, 2) === $available_language)
					   || (substr($available_language, 0, 2) === $preferred_language)) {
						if (!self::$language) {
							self::$language = $available_language;
						}
						return $available_language;
