...
class Sharees {
	...
	protected function getUsers($search) {
		...
		if (!$this->shareeEnumeration) {
			...
			$addressBookContacts = array_filter($addressBookContacts, function($contact) {
				if (isset($contact['isLocalSystemBook'])) {
					return $contact['isLocalSystemBook'] === 0;
				} else {
					return true;	
				}
			});
			$addressBookFns = array_map(function($contact) { return strtolower($contact['FN']); }, $addressBookContacts);
			$addressBookMails = array_map(function($contact) { return strtolower($contact['EMAIL']); }, $addressBookContacts);
			$this->result['users'] = array_filter($this->result['users'], function($user) use ($addressBookFns, $addressBookMails) {
				return in_array(strtolower($user['value']['shareWith']), $addressBookMails) ||
					in_array(strtolower($user['label']), $addressBookFns);
			});
		}
	}
	...
	protected function getRemote($search) {
		...
		if (!$foundRemoteById && substr_count($search, '@') >= 2 && $this->offset === 0) {
			$this->result['exact']['remotes'][] = [
				'label' => $search,
				'value' => [
					'shareType' => Share::SHARE_TYPE_REMOTE,
